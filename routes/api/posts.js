...
// Bringing the Post model
const Post = require('../../models/Post');
// Bringing the Profile model
...
router.get('/:id', (req, res) => {
    Post.findById(req.params.id)   // <-- remember to put req.params because findById require argument
...
router.post('/like/:id', passport.authenticate('jwt', { session: false }), (req, res) => {
    // find the user, get the profile, find the post by the ID, we will get the post
    Profile.findOne({ user: req.user.id })
        .then(profile => {
            Post.findById(req.params.id)
                .then(post => {
...
// LECTURE #28 ADD & REMOVE COMMENTS
